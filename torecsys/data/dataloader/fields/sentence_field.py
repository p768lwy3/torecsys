...
class SentenceField(object):
    ...
    def __init__(self, 
        ...
        Args:
            tokenize (Callable[[str], list], optional): function to split input sentences to list of string. Defaults to lambdax:x.split().
    ...
    def build_vocab(self, dataset: List[str]):
        ...
        cnts = dict(Counter(tok for sent in dataset for tok in self.tokenize(sent)))
    ...
    def to_index(self, inputs: List[str]) -> torch.Tensor:
        r"""Return corresponding indicies of tokens by inputing a list of sentence strings
        ...
        outputs = []
        for sent in inputs:
            sent = [self.vocab_dict[t] if t in self.vocab_dict else self.vocab_dict[self.unk_token] for t in self.tokenize(sent)]
            outputs.append(sent)
        
        perm_tuple = [(c, s) for c, s in sorted(zip(outputs, range(len(outputs))), key = lambda x: len(x[0]), reverse=True)]
    ...
    def from_index(self, 
        ...
        # convert tensor to numpy, then take vectorize mapping
        inputs = inputs.numpy()
