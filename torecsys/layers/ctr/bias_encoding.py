...
class BiasEncodingLayer(nn.Module):
    ...
    Bias Encoding was used in Deep Session Inerest Network :title:`Yufei Feng et al, 2019`[1], 
    ...
    def __init__(self, 
        ...
        Attributes:
            session_bias (nn.Paramter): Bias variable of session in sequence.
            position_bias (nn.Paramter): Bias variable of position in session.
            item_bias (nn.Paramter): Bias variable of embedding features in item.
        """
    ...
    def forward(self, session_embed_inputs: Tuple[torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        Args:
            session_embed_inputs ((T, T)), shape = ((B, L, E), (B, )), dtype = (torch.float, torch.long): Embedded feature tensors of session and Position of session in sequence.
